...
def get_google_api_key() -> str:
    """Fetch the Google API key for any Google/Gemini features (not used by chatbot)."""
    try:
...
def get_chatbot() -> FitnessChatbot:
    """
    Instantiate the Mistral chatbot once per session.
    Key resolution happens inside FitnessChatbot — do NOT pass Google key here.
    """
...
def render_header() -> None:
    # NOTE: set_page_config has been moved to main() where it runs first.
    st.title(f"🏋️ {APP_TITLE}")
    st.caption(
        "Live pose overlay · Single-arm & multi-angle rep counting · Mistral fitness chat"
    )
...
def chatbot_tab() -> None:
    st.subheader("Fitness chatbot (Mistral)")
    bot = get_chatbot()
...
def main() -> None:
    # MUST be the absolute first Streamlit command — nothing else before this
    st.set_page_config(
...
#     """
#     Instantiate the Mistral chatbot once per session.
#     The FitnessChatbot constructor reads MISTRAL_API_KEY from
#     Streamlit secrets or the environment automatically — do NOT
#     pass the Google key here.
#     """
#     if st.session_state.chatbot is None:
#         st.session_state.chatbot = FitnessChatbot()  # key resolved inside the class
#     return st.session_state.chatbot
